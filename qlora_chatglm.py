...
class TrainingArguments(transformers.Seq2SeqTrainingArguments):
    ...
    flag: str = field(default='chatglm', metadata={"help": 'The flg for the model.'})
    lora_r: int = field(
        default=64,
...
def get_accelerate_model(args, checkpoint_dir):
    ...
    config = AutoConfig.from_pretrained(args.model_name_or_path, device_map='auto', trust_remote_code=True)
...
class DataCollatorForCausalLM(object):
    ...
    def __call__(self, instances: Sequence[Dict]) -> Dict[str, torch.Tensor]:
        ...
        tokenized_sources_with_prompt = self.tokenizer(
            sources,
            max_length=self.source_max_len - 1 if self.flag == "chatglm" else self.source_max_len,
        ...
        for tokenized_source, tokenized_target in zip(
            ...
            if not self.predict_with_generate:
                ...
                if not self.train_on_source:
                    ...
                    labels.append(
                        tmp_label if self.flag in ["chatglm", "chinese_bloom"] else torch.tensor(tmp_label))
                else:
        ...
        if self.tokenizer.padding_side == "right" and self.flag != 'chinese_bloom':
        ...
        elif self.flag == "chinese_bloom":
            data_dict = {
                'input_ids': input_ids,
                "attention_mask": attention_mask,
            }
        elif self.flag == "chinese_alpaca":
            data_dict = {
                'input_ids': input_ids,
            }
        else:
...
def extract_alpaca_dataset(example, flag):
    ...
    example2 = format_example(example)
    return {'input': prompt_format.format(**example2)}
...
def make_data_module(tokenizer: transformers.PreTrainedTokenizer, args) -> Dict:
    ...
    elif args.dataset == 'estate':
        dataset = load_dataset("json", data_files="./data/estate_qa.json")
        dataset = dataset.map(lambda x: extract_alpaca_dataset(x, args.flag), remove_columns=['instruction'])
